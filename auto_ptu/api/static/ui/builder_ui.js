(function () {
  ...
  function _buildDeckCard(entry, evalResult) {
  ...
  function _buildSlotCard(name, handlers) {
  ...
  function renderPokeEdgeBoard(config) {
    ...
    deckTitle.textContent = "Card Deck";
    ...
    slotTitle.textContent = "Slot Board";
    slotPanel.appendChild(slotTitle);
    const slotMeta = document.createElement("div");
    slotMeta.className = "char-row-meta";
    slotMeta.textContent = "Drag cards from deck to slots. Invalid drops show a ghost warning.";
    ...
    ordered.forEach((name) => {
      slots.appendChild(_buildSlotCard(name, { onRemove }));
    });

    const minimumSlots = Math.max(6, ordered.length + 1);
    for (let i = ordered.length; i < minimumSlots; i += 1) {
      const empty = document.createElement("div");
      empty.className = "char-slot-empty";
      empty.textContent = "Drop Poke Edge Here";
