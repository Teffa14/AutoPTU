"""Generate lookup data identifying Foundry moves that carry the `contact` trait."""
...
OUTPUT_PATH = PROJECT_ROOT / "auto_ptu" / "data" / "compiled" / "contact_moves.json"
        ...
        for entry in entries:
            ...
            for action in system.get("actions", []) or []:
                for trait in action.get("traits", []) or []:
                    if str(trait).strip().lower() == "contact":
                        contact_moves.add(_normalize(name))
    OUTPUT_PATH.parent.mkdir(parents=True, exist_ok=True)
    with OUTPUT_PATH.open("w", encoding="utf-8") as out:
        json.dump(sorted(contact_moves), out, indent=2)
    print(f"Wrote {len(contact_moves)} contact moves to {OUTPUT_PATH.relative_to(PROJECT_ROOT)}")
