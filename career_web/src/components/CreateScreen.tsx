...
export function CreateScreen({ locale, onCreated }: Props) {
  ...
  const [starter, setStarter] = useState("Rattata");
  ...
  function chooseRegion(value: string) {
    setRegion(value);
    const next = catalog?.regions.find((entry) => entry.id === value)?.underdogs[0];
      ...
      {!catalog ? <div className="ticket-loader">Preparing the regional registry…</div> : (
        <form className="registration-book" onSubmit={begin}>
          ...
          <fieldset className="starter-field">
            <legend>{locale === "es" ? "Elige al que nadie eligió" : "Choose the one nobody chose"}</legend>
            <div className="starter-strip">
              {selectedRegion?.underdogs.map((species) => (
                <label key={species} className={starter === species ? "starter-card selected" : "starter-card"}>
                  <input type="radio" name="starter" checked={starter === species} onChange={() => setStarter(species)} />
                  <PokemonSprite name={species} className="starter-sprite" />
                  <b>{species}</b><small>underdog · regional</small>
                </label>
              ))}
            </div>
          </fieldset>
