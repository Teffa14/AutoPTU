    ...
    <section className="daily-scene">
      ...
      <aside className="ranked-entry">
        ...
        <div className="daily-build-fields">
          ...
          <label><span>{locale === "es" ? "Clase PTU" : "PTU class"}</span><select value={trainerClass} onChange={(event) => setTrainerClass(event.target.value)}>{catalog?.classes.map((entry) => <option key={entry.id} value={entry.name}>{entry.name}</option>)}</select><small>{catalog?.classes.find((entry) => entry.name === trainerClass)?.[locale === "es" ? "description_es" : "description_en"]}</small></label>
        </div>
