...
OUT_BUILDER_JSON = ROOT / "auto_ptu" / "api" / "static" / "AutoPTUCharacter" / "pokedex_learnset.json"
OUT_BUILDER_EMBED = ROOT / "auto_ptu" / "api" / "static" / "AutoPTUCharacter" / "pokedex_learnset.embed.js"
...
def main() -> None:
    ...
    for path in (OUT_STATIC_JSON, OUT_BUILDER_JSON):
        path.write_text(json_text, encoding="utf-8")
    for path in (OUT_STATIC_EMBED, OUT_BUILDER_EMBED):
        path.write_text(embed_text, encoding="utf-8")
    print(
        f"Wrote learnset payloads for {len(payload)} species/forms to "
        f"{OUT_STATIC_JSON}, {OUT_STATIC_EMBED}, {OUT_BUILDER_JSON}, and {OUT_BUILDER_EMBED}."
    )
