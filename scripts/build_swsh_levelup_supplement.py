...
OUT_JSON = ROOT / "auto_ptu" / "data" / "compiled" / "swsh_levelup_learnsets.json"
USER_AGENT = "AutoPTU-SwShLearnsetBuilder/1.0"
POKEAPI_POKEMON = "https://pokeapi.co/api/v2/pokemon/"
POKEAPI_SPECIES = "https://pokeapi.co/api/v2/pokemon-species/"
SWSH_VERSION_GROUPS = {"sword-shield", "the-isle-of-armor", "the-crown-tundra"}
...
def _swsh_level_up_moves(pokemon_payload: dict[str, Any]) -> list[dict[str, Any]]:
    best: dict[str, int] = {}
    for move_entry in pokemon_payload.get("moves", []):
        ...
        for detail in move_entry.get("version_group_details", []):
            if str(detail.get("version_group", {}).get("name") or "").strip().lower() not in SWSH_VERSION_GROUPS:
                continue
...
def main() -> None:
    ...
    payload_entries: dict[str, list[dict[str, Any]]] = {}
    for entry in entries:
        pokemon_name = chosen_pokemon_by_entry[entry["name"]]
        moves = _swsh_level_up_moves(pokemon_payloads[pokemon_name])
        if moves:
            payload_entries[entry["name"]] = moves

    payload = {
        "generated_from": "PokeAPI Sword/Shield + Isle of Armor + Crown Tundra version groups",
